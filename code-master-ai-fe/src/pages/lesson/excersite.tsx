...
function AiTutorModal({
  isOpen, onClose, aiHint, isLoading,
}: { isOpen: boolean; onClose: () => void; aiHint: string | null; isLoading: boolean }) {
  if (!isOpen) return null;
  return (
    <div
      style={{ position: "fixed", inset: 0, zIndex: 50, display: "flex", alignItems: "center", justifyContent: "center", background: "rgba(0,0,0,0.45)", backdropFilter: "blur(4px)" }}
      onClick={onClose}
    >
      <div
        style={{ width: "100%", maxWidth: 520, margin: "0 16px", background: "white", borderRadius: 16, boxShadow: "0 20px 60px rgba(0,0,0,0.15)", overflow: "hidden" }}
      ...
      >
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "16px 20px", borderBottom: "1px solid #f1f5f9" }}>
          <div style={{ width: 36, height: 36, borderRadius: 10, background: "#ede9fe", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
            <svg width="16" height="16" viewBox="0 0 24 24" fill="#7c3aed"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2" /></svg>
          </div>
          <div style={{ flex: 1 }}>
            <p style={{ fontWeight: 600, fontSize: 14, color: "#0f172a", margin: 0 }}>Gia Sư AI</p>
            <p style={{ fontSize: 11, color: "#94a3b8", margin: 0 }}>Phân tích lỗi và gợi ý cải thiện</p>
          </div>
          ...
          {isLoading ? (
            <div>
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 16 }}>
                ...
                <span style={{ fontSize: 12, color: "#7c3aed" }}>Gia sư AI đang phân tích code của bạn...</span>
              </div>
              {[100, 80, 95, 65].map((w, i) => (
                <div key={i} style={{ height: 10, background: "#f1f5f9", borderRadius: 4, marginBottom: 10, width: `${w}%`, animation: "pulse 1.5s infinite" }} />
              ))}
            </div>
          ) : aiHint ? (
            <p style={{ fontSize: 13, color: "#334155", lineHeight: 1.7, whiteSpace: "pre-wrap", margin: 0 }}>{aiHint}</p>
          ) : (
            <p style={{ fontSize: 13, color: "#94a3b8", margin: 0 }}>Không có gợi ý.</p>
          )}
        </div>

        {!isLoading && (
          <div style={{ padding: "12px 20px", borderTop: "1px solid #f1f5f9", display: "flex", justifyContent: "flex-end" }}>
            <button onClick={onClose} style={{ fontSize: 12, padding: "7px 18px", borderRadius: 8, border: "1px solid #e2e8f0", background: "white", color: "#64748b", cursor: "pointer", fontWeight: 500 }}>
              Đóng
            </button>
          </div>
        )}
...
export default function ExercisePage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const ts2 = new Date().toLocaleTimeString();
      if (data.compileError) {
        setConsoleOutput(`[${ts2}] Compiling project...\n✗ Compilation failed\n\n${data.compileError}`);
      } else {
  ...
  const handleAskAiTutor = async () => {
    ...
    if (!subId) {
      setAiHint("Không tìm thấy ID bài nộp (có thể API giả lập hoặc hệ thống chưa trả về ID). Vui lòng nộp bài lại.");
      return;
    }
  ...
  const fileExt: Record<string, string> = {
    javascript: "jsx", typescript: "tsx", python: "py", java: "java", cpp: "cpp",
  };
  ...
  return (
    <>
      ...
      <div style={{ display: "flex", height: "100vh", background: "#f8fafc", fontFamily: "'Geist', 'DM Sans', system-ui, sans-serif", fontSize: 13, overflow: "hidden", userSelect: "none" }}>

        {/* ===== LEFT PANEL: ĐỀ BÀI ===== */}
        <div style={{ width: "36%", minWidth: 300, background: "white", borderRight: "1px solid #e8edf2", display: "flex", flexDirection: "column", overflow: "hidden" }}>

          {/* Title area */}
          <div style={{ padding: "20px 22px 0" }}>
            <h1 style={{ fontSize: 17, fontWeight: 700, color: "#0f172a", lineHeight: 1.35, marginBottom: 10 }}>
              {loadingExercise ? (
                <div style={{ height: 20, background: "#f1f5f9", borderRadius: 6, width: "75%", animation: "pulse 1.5s infinite" }} />
              ) : `Bài tập: ${title}`}
            </h1>

            {/* Tabs */}
            <div style={{ display: "flex", gap: 0, borderBottom: "1.5px solid #f1f5f9" }}>
          ...
          <div style={{ flex: 1, overflowY: "auto", padding: "18px 22px" }}>
              ...
              loadingExercise ? (
                <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                  {[75, 55, 85, 45, 65].map((w, i) => (
                    <div key={i} style={{ height: 11, background: "#f1f5f9", borderRadius: 4, width: `${w}%` }} />
                  ))}
                </div>
              ) : (
                <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
                  ...
                  <p style={{ fontSize: 13, color: "#475569", lineHeight: 1.7, margin: 0 }}>
                    {exercise?.description ?? (
                      <>
                        ...
                        <code style={{ background: "#f1f5f9", padding: "1px 6px", borderRadius: 4, fontSize: 12, color: "#0891b2", fontFamily: "monospace" }}>Greeting</code>
                        . Component này sẽ nhận vào một prop là{" "}
                        <code style={{ background: "#f1f5f9", padding: "1px 6px", borderRadius: 4, fontSize: 12, color: "#0891b2", fontFamily: "monospace" }}>name</code>
                        {" "}và hiển thị lời chào tương ứng.
                      </>
                    )}
                  </p>
                  ...
                  {/* Requirements */}
                  <div>
                    <p style={{ fontSize: 11, fontWeight: 700, letterSpacing: "0.06em", color: "#94a3b8", textTransform: "uppercase", marginBottom: 10 }}>Yêu cầu:</p>
                  ...
                  {/* Examples */}
                  <div>
                    <p style={{ fontSize: 11, fontWeight: 700, letterSpacing: "0.06em", color: "#94a3b8", textTransform: "uppercase", marginBottom: 10 }}>Ví dụ:</p>
                    <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                      {(exercise?.examples ?? [
                        { input: '<Greeting name="An" />', output: "<h1>Xin chào, An!</h1>" },
                        { input: "<Greeting />", output: "<h1>Xin chào, Khách!</h1>" },
                      ]).map((ex, i) => (
                        <div key={i} style={{ background: "#f8fafc", border: "1px solid #e8edf2", borderRadius: 10, padding: "10px 14px", fontFamily: "monospace", fontSize: 12 }}>
                          <div style={{ color: "#0891b2", marginBottom: 4 }}>
                            <span style={{ color: "#94a3b8", fontSize: 10, fontFamily: "sans-serif", marginRight: 6 }}>INPUT</span>
                            {ex.input}
                          </div>
                          <div style={{ color: "#64748b" }}>
                            <span style={{ color: "#94a3b8", fontSize: 10, fontFamily: "sans-serif", marginRight: 6 }}>OUTPUT</span>
                            {ex.output}
                          </div>
                        </div>
                      ))}
            ...
            {leftTab === "hint" && (
              <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 12, paddingTop: 32, color: "#94a3b8", textAlign: "center" }}>
                <div style={{ width: 48, height: 48, borderRadius: "50%", background: "#ede9fe", display: "flex", alignItems: "center", justifyContent: "center" }}>
                  <svg width="22" height="22" viewBox="0 0 24 24" fill="#7c3aed"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2" /></svg>
                </div>
                <p style={{ fontSize: 13, color: "#475569", margin: 0 }}>Nộp bài trước để nhận gợi ý từ AI</p>
                {showAiTutorBtn && (
                  <button onClick={handleAskAiTutor} style={{
            ...
            {leftTab === "discuss" && (
              <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 8, paddingTop: 32, color: "#94a3b8", textAlign: "center" }}>
                ...
                <p style={{ fontSize: 13, color: "#94a3b8", margin: 0 }}>Chưa có thảo luận nào</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="px-2" style={{ flex: 1, display: "flex", flexDirection: "column", gap: 16, overflow: "hidden",background:"white" }}>

          {/* Editor block rounded */}
          <div style={{ flex: 1, display: "flex", flexDirection: "column", background: "white", borderRadius: 16, overflow: "hidden", border: "1px solid #e2e8f0", minHeight: 0 }}>
             {/* Dark editor header */}
             <div className="shadow-lg" style={{ background: "#252526", height: 42, display: "flex", alignItems: "center", padding: "0 16px", flexShrink: 0 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#61dafb" strokeWidth="2"><polyline points="16 18 22 12 16 6" /><polyline points="8 6 2 12 8 18" /></svg>
                  <span style={{ fontSize: 13, color: "#d4d4d4", fontFamily: "'JetBrains Mono', monospace" }}>
                    greeting.{fileExt[language] ?? "jsx"}
                  </span>
                  
                  {/* Language Selector in Light Theme */}
                  <div style={{ marginLeft: 16, height: 24, borderLeft: "1px solid #3f3f46", paddingLeft: 14, display: "flex", alignItems: "center" }}>
                    <Select
                      value={language}
          ...
          {/* Action Bar */}
          <div className="shadow-lg" style={{ display: "flex", alignItems: "center", justifyContent: "space-between", background: "white", borderRadius: 999, padding: "10px 24px", border: "1px solid #e2e8f0", flexShrink: 0, boxShadow: "0 2px 4px rgba(0,0,0,0.02)" }}>
             <div style={{ display: "flex", alignItems: "center", gap: 32 }}>
                <button
                  onClick={handleSubmit}
                  disabled={isSubmitting}
                  style={{ display: "flex", alignItems: "center", gap: 8, background: "none", border: "none", fontSize: 14, fontWeight: 600, color: "#475569", cursor: isSubmitting ? "not-allowed" : "pointer", opacity: isSubmitting ? 0.5 : 1 }}
                >
                ...
                <button
                  onClick={handleReset}
                  style={{ display: "flex", alignItems: "center", gap: 8, background: "none", border: "none", fontSize: 14, fontWeight: 600, color: "#475569", cursor: "pointer" }}
                >
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><polyline points="1 4 1 10 7 10" /><path d="M3.51 15a9 9 0 1 0 .49-4.9" /></svg>
                  Làm lại
                </button>
             </div>
             ...
             <button
                ...
                style={{
                  ...
                  fontSize: 14, padding: "10px 28px", borderRadius: 999,
                  border: "none", background: "#4e7c5e",
                  color: "white", cursor: isSubmitting ? "not-allowed" : "pointer",
                  fontWeight: 600, opacity: isSubmitting ? 0.7 : 1, transition: "background 0.15s"
                }}
             >
                {isSubmitting ? (
                  <>
                    <span style={{ width: 12, height: 12, border: "2px solid rgba(255,255,255,0.3)", borderTopColor: "white", borderRadius: "50%", animation: "spin 0.7s linear infinite", display: "inline-block" }} />
                    Đang chấm...
                  </>
                ) : (
                  <>
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><polyline points="22 2 11 13" /><polygon points="22 2 15 22 11 13 2 9 22 2" /></svg>
                    Nộp bài
                  </>
                )}
             </button>
          ...
          {/* Console Card */}
          <div className="shadow-lg" style={{ height: 180, display: "flex", flexDirection: "column", background: "#111827", borderRadius: 16, overflow: "hidden", flexShrink: 0 }}>
             <div style={{ display: "flex", alignItems: "center", background: "#1e2530", borderBottom: "1px solid #2d3748", padding: "0 16px", height: 42, flexShrink: 0 }}>
               <div style={{ display: "flex", alignItems: "center", gap: 8, marginRight: 24 }}>
                 <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#64748b" strokeWidth="2"><polyline points="4 17 10 11 4 5" /><line x1="12" y1="19" x2="20" y2="19" /></svg>
                 <span style={{ fontSize: 11, fontWeight: 700, letterSpacing: "0.08em", color: "#64748b", textTransform: "uppercase" }}>Console output</span>
               </div>
               ...
               {(["console", "result"] as const).map((tab) => (
                  <button
                    ...
                    onClick={() => setActiveTab(tab)}
                    style={{
                      fontSize: 13, padding: "0 16px", height: "100%",
                      background: "none", border: "none",
                      borderBottom: activeTab === tab ? "2px solid #22c55e" : "2px solid transparent",
                      color: activeTab === tab ? "#f1f5f9" : "#64748b",
                      cursor: "pointer", fontWeight: activeTab === tab ? 600 : 500,
                  ...
                  >
                    ...
                    {tab === "result" && result && (
                      <span style={{
                        ...
                        background: result.submission?.status === "ACCEPTED" ? "#dcfce7" : "#fee2e2",
                        color: result.submission?.status === "ACCEPTED" ? "#15803d" : "#b91c1c",
                        fontWeight: 700,
                      }}>
                        {result.submission?.status === "ACCEPTED" ? "AC" : "WA"}
                      </span>
             ...
             <div style={{ flex: 1, overflowY: "auto", padding: "12px 16px" }}>
               {activeTab === "console" && (
                 <pre style={{ fontFamily: "'JetBrains Mono', 'Fira Code', Consolas, monospace", fontSize: 12, lineHeight: 1.8, margin: 0, whiteSpace: "pre-wrap" }}>
                   {consoleOutput.split("\n").map((line, i) => {
                     let color = "#64748b";
                     if (line.includes("✓") || line.includes("successfully") || line.includes("passed")) color = "#4ade80";
                     else if (line.includes("✗") || line.includes("failed") || line.includes("Lỗi")) color = "#f87171";
                     else if (line.includes("Compiling") || line.includes("Đang")) color = "#60a5fa";
               ...
               {activeTab === "result" && (
                 <div style={{ color: "#f1f5f9" }}>
                   {!result ? (
                     <span style={{ fontSize: 12, color: "#475569" }}>Chưa có kết quả.</span>
                   ) : (
                     <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                       {/* Status + Score */}
                       <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                         <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                           <span style={{ fontSize: 11, color: "#64748b" }}>Trạng thái:</span>
                           <span style={{
                             fontSize: 11, fontWeight: 700, padding: "3px 10px", borderRadius: 20,
                             background: STATUS_COLOR[result.submission?.status]?.bg ?? "#f1f5f9",
                             color: STATUS_COLOR[result.submission?.status]?.text ?? "#64748b",
                           }}>
                             {STATUS_LABEL[result.submission?.status] ?? result.submission?.status?.replace(/_/g, " ")}
                           </span>
                         </div>
                       ...
                       <div>
                         <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 5, fontSize: 11, color: "#64748b" }}>
                           <span>Test cases</span>
                           <span>
                             <span style={{ color: "#f1f5f9", fontWeight: 600 }}>{result.passedCases}</span>
                             /{result.totalCases} ({pct}%)
                           </span>
                         </div>
                         <div style={{ height: 6, background: "#1e2530", borderRadius: 4, overflow: "hidden" }}>
                           <div style={{
                       ...
                       {/* Test cases list */}
                       <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "4px 12px", borderTop: "1px solid #1e2530", paddingTop: 8 }}>
                         {Array.from({ length: result.totalCases }).map((_, i) => {
                           const passed = i < result.passedCases;
                           return (
                             <div key={i} style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 11 }}>
                               <span style={{ width: 7, height: 7, borderRadius: "50%", flexShrink: 0, background: passed ? "#22c55e" : "#ef4444" }} />
          ...
          {isSuccess && (
            <div style={{
              ...
              border: "1px solid #bbf7d0", borderRadius: 100, flexShrink: 0,
            }}>
              ...
              <span style={{ fontSize: 14, color: "#15803d", flex: 1, fontWeight: 600 }}>
                Tuyệt vời! Bạn đã hoàn thành bài tập này.
              </span>
              <button
                onClick={() => navigate(-1)}
