...
export class SubmissionsService {
  ...
  async submitCode(
    ...
    try {
      ...
      const codeAssignment = await this.codeAssignmentModel.findOne({
        _id: objectId,
      });
      if (!codeAssignment) {
        throw new BadRequestException(
          'Không tìm thấy cấu hình chấm code cho bài tập này.',
        );
      }
      ...
      if (!testCases || testCases.length === 0) {
        throw new BadRequestException(
          'Bài tập này chưa có Test Case nào để chấm điểm.',
        );
      }
      ...
      if (!languageId) {
        throw new BadRequestException(
          `Hệ thống chưa hỗ trợ ngôn ngữ: ${language}`,
        );
      }
      ...
      const judgePromises = testCases.map(async (testCase: any) => {
        const payload = {
          ...
          stdin: testCase.input_data
            ? Buffer.from(testCase.input_data).toString('base64')
            : null,
        ...
        if (result.status.id === 3) {
          ...
          if (result.status.id === 6) {
            ...
            if (!errorDetail && result.stderr) {
              errorDetail = Buffer.from(result.stderr, 'base64').toString(
                'utf-8',
              );
            }
          }
        }
        if (result.time && parseFloat(result.time) > maxTime)
  ...
  async getRecommendationsForUser(userId: string) {
    ...
    if (weakTags.length === 0) {
      const easyAssignments = await this.codeAssignmentModel
    ...
    try {
      ...
      const allAssignmentsRaw = await this.codeAssignmentModel
        .find()
        .select('_id tags difficulty')
        .lean()
        .exec();
      ...
      if (!recommendedIds || recommendedIds.length === 0) {
        const backupAssignments = await this.codeAssignmentModel
          .find({ difficulty: 'EASY' })
          .limit(5)
          .exec();
      ...
      const fallbackAssignments = await this.codeAssignmentModel
        .find({ difficulty: 'EASY' })
        .limit(5)
        .exec();
